...
class LightGesture:
    ...
    def light_simulation(self, img, lights):
        ...
        # Draw the circles representing the lights
        on_color = (0, 255, 255)  # Yellow for "on"
        off_color = (0, 0, 0)    # Black for "off"
        colors = [off_color, on_color]
        circle_centers = [circle1_center, circle2_center, circle3_center]

        for center, light in zip(circle_centers, lights):
            color = colors[int(light)]
            cv2.circle(white_rect, center, circle_radius, color, -1)

        # Append the white rectangle with lights' status to the bottom of the image
        img = np.vstack((img, white_rect))
        return img
...
if __name__ == "__main__":
    ...
    model_path = "./models/model_04-12 22:21_NeuralNetwork_best"
